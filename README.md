📊 Transactions Data Analysis Dashboard
Overview
Built a formula-driven Excel/Google Sheets model to clean, standardize, and analyze a high-variance transactional dataset. The project focuses on resolving data inconsistencies, engineering accurate financial metrics, and generating reliable revenue and customer lifecycle insights.
...
Resolved duplicate transaction IDs and handled negative/invalid quantities.

Normalized case-sensitive fields (Customer, Account Manager, Billing Cycle).

Financial Modeling & Revenue Analytics
Built dynamic calculations for Net Amount, GST (18%), and Final Amount across tax-inclusive and exclusive cases.

Developed a fully automated Month-over-Month (MoM) growth model.

Implemented custom logic to correctly aggregate fragmented month-end transactions, avoiding double-counting and broken trend lines.

Customer Lifecycle Insights
Classified transactions into Active, Churned, Paused, and Refunded states.

Identified "Hybrid" customers (mixed lifecycle states) to highlight retention risks and billing inconsistencies.

Regional Performance Intelligence
Created pivot-based summaries for:
...
Logic Handling: Nested IFs, complex array matching, and error handling (IFERROR)

Data Cleaning: TRIM, UPPER, text normalization

Automation: Dynamic arrays for fully automated, manual-free reporting
...
Calculates growth between consecutive months with safe error handling for zero/blank previous values.
